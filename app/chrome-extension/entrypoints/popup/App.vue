...
const diagnosticsTime = computed(() => !diagnostics.value?.generatedAt ? '—' : new Intl.DateTimeFormat('ar', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(diagnostics.value.generatedAt)));
...
async function runDiagnostics() { if (diagnosticsBusy.value) return; diagnosticsBusy.value = true; try { const response = await chrome.runtime.sendMessage({ type: 'brauzio_diagnostics_run' }); if (!response?.success || !response.report) throw new Error(response?.error || 'تعذر تشغيل فحص Brauzio'); diagnostics.value = response.report as DiagnosticReport; } catch (error) { diagnostics.value = { generatedAt: Date.now(), overall: 'error', extensionVersion: version, checks: [{ key: 'self-test', label: 'Self Test', state: 'error', detail: error instanceof Error ? error.message : String(error) }] }; } finally { diagnosticsBusy.value = false; } }
...
<template>
  <div class="brauzio" dir="rtl">
    ...
    <main class="main">
      ...
      <section class="diagnostics-card" :class="diagnostics ? `diag-${diagnostics.overall}` : ''"><div class="section-title"><div><span class="label">Diagnostics</span><h3>فحص Brauzio</h3></div><span class="diag-summary" :class="diagnostics ? `state-${diagnostics.overall}` : ''">{{ diagnosticsLabel }}</span></div><p class="diagnostics-intro">يفحص الاتصال وWebSocket والمصادقة وChrome Debugger وCDP وحالة الإدخال بدون تغيير الصفحة.</p><button class="button secondary full" :disabled="diagnosticsBusy" @click="runDiagnostics">{{ diagnosticsBusy ? 'جارٍ الفحص…' : diagnostics ? 'إعادة الفحص' : 'تشغيل الفحص' }}</button><div v-if="diagnostics" class="diagnostics-results"><div v-for="check in diagnostics.checks" :key="check.key" class="diagnostic-row"><span class="diag-icon" :class="`state-${check.state}`">{{ diagnosticIcon(check.state) }}</span><div class="diag-copy"><div class="diag-line"><strong>{{ check.label }}</strong><span v-if="typeof check.latencyMs === 'number'" dir="ltr">{{ check.latencyMs }} ms</span></div><p>{{ check.detail }}</p></div></div><div class="diag-meta"><span>آخر فحص: {{ diagnosticsTime }}</span><span v-if="diagnostics.serverVersion" dir="ltr">Cloud v{{ diagnostics.serverVersion }}</span><span v-if="diagnostics.toolCount">{{ diagnostics.toolCount }} tools</span></div></div></section>
