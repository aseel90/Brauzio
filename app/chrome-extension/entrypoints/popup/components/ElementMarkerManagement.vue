<template>
  <div class="section">
    <h2 class="section-title">إدارة تحديد العناصر</h2>
    <div class="config-card">
      <div class="status-section" style="gap: 8px">
        <div class="status-header">
          <p class="status-label">الصفحة الحالية</p>
          <span class="status-text" style="opacity: 0.85">{{ currentUrl }}</span>
        </div>
        <div class="status-header">
          <p class="status-label">العناصر المحددة</p>
      ...
      <form class="mcp-config-section" @submit.prevent="onAdd">
        <div class="mcp-config-header">
          <p class="mcp-config-label">إضافة تحديد</p>
        </div>
        <div style="display: flex; gap: 8px; margin-bottom: 8px">
          <input v-model="form.name" placeholder="الاسم، مثال: زر تسجيل الدخول" class="port-input" />
        ...
        <input v-model="form.selector" placeholder="محدد CSS" class="port-input" />
        <div style="display: flex; gap: 8px; margin-top: 8px">
          <button class="semantic-engine-button" :disabled="!form.selector" type="submit">
            حفظ
          </button>
          <button class="danger-button" type="button" @click="resetForm">مسح</button>
        </div>
      </form>
      ...
      <div v-if="markers.length" class="model-list" style="margin-top: 8px">
        ...
        >
          ...
          <div style="display: flex; gap: 6px">
            <button class="semantic-engine-button" @click="validate(m)">تحقق</button>
            <button class="secondary-button" @click="prefill(m)">تعديل</button>
            <button class="danger-button" @click="remove(m)">حذف</button>
          </div>
        </div>
      </div>
    </div>
