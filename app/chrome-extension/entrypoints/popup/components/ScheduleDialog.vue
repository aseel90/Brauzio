<template>
  <div v-if="visible" class="rr-modal">
    <div class="rr-dialog">
      <div class="rr-header">
        <div class="title">تشغيل مجدول</div>
        <button class="close" @click="$emit('close')">✕</button>
      </div>
      <div class="rr-body">
        <div class="row">
          <label>تفعيل</label>
          <label class="chk"><input type="checkbox" v-model="enabled" />تفعيل الجدولة</label>
        </div>
        <div class="row">
          <label>النوع</label>
          <select v-model="type">
            <option value="interval">كل N دقيقة</option>
            <option value="daily">يوميًا في وقت محدد</option>
            <option value="once">تشغيل مرة واحدة فقط</option>
          </select>
        </div>
        <div class="row" v-if="type === 'interval'">
          <label>الفاصل (بالدقائق)</label>
          <input type="number" v-model.number="intervalMinutes" />
        </div>
        <div class="row" v-if="type === 'daily'">
          <label>الوقت (HH:mm)</label>
          <input v-model="dailyTime" placeholder="مثال: 09:30" />
        </div>
        <div class="row" v-if="type === 'once'">
          <label>الوقت (ISO)</label>
          <input v-model="onceAt" placeholder="مثال: 2025-10-05T10:00:00" />
        </div>
        <div class="row">
        ...
        <div class="section">
          <div class="section-title">الجداول الحالية</div>
          <div class="sched-list">
            <div class="sched-row" v-for="s in schedules" :key="s.id">
              ...
              <div class="actions">
                <button class="small danger" @click="$emit('remove', s.id)">حذف</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="rr-footer">
        <button class="primary" @click="save">حفظ</button>
      </div>
    </div>
  </div>
</template>
...
function describe(s: any) {
  if (s.type === 'interval') return `كل ${s.when} دقيقة`;
  if (s.type === 'daily') return `يوميًا ${s.when}`;
  if (s.type === 'once') return `مرة واحدة ${s.when}`;
  return '';
}
</script>
