<template>
  ...
  <div v-if="cfg.modes.command" class="form-section">
    ...
    <div class="text-xs text-slate-500" style="padding: 0 20px"
      >ملاحظة: يجب تعريف اختصار إضافة Chrome مسبقًا في manifest ولا يمكن إضافته ديناميكيًا أثناء التشغيل.</div
    >
  </div>
  ...
  <div class="form-section">
    <div class="text-xs text-slate-500" style="padding: 0 20px"
      >توضيح: 
      تتم مزامنة المشغل عند حفظ سير العمل مع مشغلات الخلفية (URL/القائمة السياقية/الاختصار/DOM) والمهام المجدولة (فاصل/يومي/مرة واحدة).
    </div>
  </div>
</template>
