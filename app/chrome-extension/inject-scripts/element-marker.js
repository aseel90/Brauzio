...
(function () {
    ...
    const PANEL_TEMPLATE = `
      <div class="em-panel" id="em_panel_root">
        ...
        <div class="em-header em-drag-handle" id="__em_drag_handle" title="Drag to move">
          <h2 class="em-title">تحديد العناصر</h2>
        ...
        <div class="em-controls">
          ...
          <button class="em-square-btn" id="__em_toggle_list" title="وضع القائمة - تحديد عناصر متشابهة دفعة واحدة (CSS فقط)">
            <svg viewBox="0 0 24 24">
    ...
    try {
      ...
      if (res?.tool?.ok) {
        StateStore.set({
          validation: {
            status: 'success',
            message: `✓ تم التحقق بنجاح (تمت مطابقة ${filteredMatches.length} عنصرًا)`,
          },
        ...
        StateStore.set({
          validation: {
            status: 'failure',
            message: res?.tool?.error || 'فشل التحقق',
          },
          validationHistory: history,
        });
      }
  ...
  function copySelectorNow() {
    try {
      ...
      StateStore.set({
        validation: { status: 'success', message: '✓ تم النسخ إلى الحافظة' },
      });
