...
(function () {
  ...
  class UI {
    constructor(recorder) {
      ...
      root.innerHTML = `
        <div id="__rr_rec_panel" style="background: rgba(220,38,38,0.95); color: #fff; padding:8px 10px; border-radius:8px; display:flex; align-items:center; gap:8px; box-shadow:0 4px 16px rgba(0,0,0,0.2);">
          <span id="__rr_badge" style="font-weight:600;">جارٍ التسجيل</span>
          <label style="display:inline-flex; align-items:center; gap:4px; font-size:12px;">
            <input id="__rr_hide_values" type="checkbox" style="vertical-align:middle;" />إخفاء قيم الإدخال
          </label>
          <label style="display:inline-flex; align-items:center; gap:4px; font-size:12px;">
            <input id="__rr_enable_highlight" type="checkbox" style="vertical-align:middle;" />تمييز
          </label>
          <button id="__rr_toggle_timeline" style="background:transparent; color:#fff; border:1px solid rgba(255,255,255,0.5); border-radius:6px; padding:2px 6px; cursor:pointer; font-size:12px;">طي</button>
          <button id="__rr_pause" style="background:#fff; color:#111; border:none; border-radius:6px; padding:4px 8px; cursor:pointer;">إيقاف مؤقت</button>
          <button id="__rr_stop" style="background:#111; color:#fff; border:none; border-radius:6px; padding:4px 8px; cursor:pointer;">إيقاف</button>
        </div>`;
      ...
      header.textContent = 'الخطوات المسجلة';
      ...
      if (btnToggle) {
        btnToggle.addEventListener('click', () => {
          ...
          btnToggle.textContent = this._collapsed ? 'توسيع' : 'طي';
        });
      }
    ...
    updateStatus() {
      ...
      if (badge) badge.textContent = this.recorder.isPaused ? 'متوقف مؤقتًا' : 'جارٍ التسجيل';
      if (pauseBtn) pauseBtn.textContent = this.recorder.isPaused ? 'متابعة' : 'إيقاف مؤقت';
    }
    ...
    _formatStepText(step, _idx) {
      try {
        if (!step || typeof step !== 'object') return 'خطوة غير معروفة';
        const t = step.type;
        const sel = step.target && step.target.selector ? step.target.selector : '';
        if (t === 'click' || t === 'dblclick') {
          return `${t === 'dblclick' ? 'نقر مزدوج' : 'نقر'}: ${sel || '(document)'}`;
        }
        if (t === 'fill') {
          const val = step.value;
          const shown = typeof val === 'string' && val.length > 0 ? val : String(val);
          return `إدخال: ${sel} = ${shown}`;
        }
        if (t === 'scroll') {
          const mode = step.mode === 'container' ? 'الحاوية' : 'الصفحة';
          const off = step.offset || {};
          return `تمرير(${mode}): y=${off.y ?? 0}, x=${off.x ?? 0}`;
        }
        if (t === 'openTab') return `فتح علامة تبويب: ${step.url || ''}`;
        if (t === 'switchTab') return `تبديل علامة التبويب: يتضمن ${step.urlContains || ''}`;
        if (t === 'switchFrame')
          return `تبديل Frame: يتضمن ${step.frame && step.frame.urlContains ? step.frame.urlContains : ''}`;
        if (t === 'waitFor') return `انتظار: ${sel || step.until || ''}`;
        return `${t}`;
      } catch (_) {
        return 'خطوة';
      }
