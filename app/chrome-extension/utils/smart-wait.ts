...
async function waitForDomCondition(
  ...
  const result = await chrome.scripting.executeScript({
    ...
    func: async (
      ...
      const check = () => {
        try {
          ...
          const hasText = (document.body?.innerText || document.documentElement?.innerText || '').includes(text);
      ...
      if (typeof initial === 'object' && initial?.error) return { ok: false, error: initial.error };

      return await new Promise<{ ok: boolean; timedOut?: boolean; error?: string }>((resolve) => {
        let finished = false;
        let observer: MutationObserver | null = null;
        let timer: ReturnType<typeof setTimeout> | null = null;

        const finish = (payload: { ok: boolean; timedOut?: boolean; error?: string }) => {
          if (finished) return;
          finished = true;
          observer?.disconnect();
        ...
        observer = new MutationObserver(() => {
          ...
          else if (typeof next === 'object' && next?.error) finish({ ok: false, error: next.error });
        });
        ...
        if (typeof afterObserve === 'object' && afterObserve?.error) {
    ...
    args: [
      options.condition as 'selector_exists' | 'selector_hidden' | 'text_appears' | 'text_disappears',
  ...
  const payload = result?.[0]?.result as { ok?: boolean; timedOut?: boolean; error?: string; immediate?: boolean } | undefined;
  if (payload?.ok) return { ok: true, details: { immediate: Boolean(payload.immediate) } };
