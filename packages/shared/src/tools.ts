...
export const TOOL_SCHEMAS: Tool[] = [
  ...
  { name: TOOL_NAMES.BROWSER.COMPUTER, description: 'Control Chrome with visible Brauzio mouse feedback. Supports true press/move/release state for canvases, games, sliders and virtual joysticks.', inputSchema: { type: 'object', properties: { ...tabTarget, action: { type: 'string', description: 'left_click | right_click | double_click | triple_click | left_click_drag | drag_hold | mouse_move | mouse_down | mouse_up | scroll | scroll_to | type | key | fill | fill_form | hover | wait | resize_page | zoom | screenshot' }, coordinates: { ...point, description: 'Target/end viewport coordinates.' }, startCoordinates: { ...point, description: 'Drag start viewport coordinates.' }, ref: { type: 'string' }, startRef: { type: 'string' }, selector: { type: 'string' }, selectorType: { type: 'string', enum: ['css', 'xpath'] }, frameId: { type: 'number' }, scrollDirection: { type: 'string', enum: ['up', 'down', 'left', 'right'] }, scrollAmount: { type: 'number' }, text: { type: 'string' }, repeat: { type: 'number' }, value: {}, elements: { type: 'array', items: { type: 'object', properties: { ref: { type: 'string' }, value: {} }, required: ['ref', 'value'] } }, duration: { type: 'number', description: 'Seconds. For drag_hold this is the hold time at the destination (max 15).' }, appear: { type: 'boolean' }, width: { type: 'number' }, height: { type: 'number' }, region: { type: 'object', properties: { x0: { type: 'number' }, y0: { type: 'number' }, x1: { type: 'number' }, y1: { type: 'number' } } } }, required: ['action'] } },
