...
raw_cdp = r'''import { createErrorResponse, type ToolResult } from '@/common/tool-handler';
...
Path('app/chrome-extension/entrypoints/background/tools/browser/raw-cdp.ts').write_text(raw_cdp)

p = Path('app/chrome-extension/entrypoints/background/tools/browser/index.ts')
s = p.read_text()
line = "export { rawCdpTool } from './raw-cdp';\n"
if line not in s:
    s += line
p.write_text(s)
